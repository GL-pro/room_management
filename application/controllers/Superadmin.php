...
class Superadmin extends CI_Controller {
	...
	public function booked_enquiry() {
		...
		// var_dump($selected_rooms, $booking_id); // This will show in your debug console or error log
	...
	foreach ($items_data as $room_id => $room_items) {
		...
		foreach ($room_items as $item) {
			...
			$data = [
				...
				'adding_date' => $adding_date,
				'admin_status' => 'staff',
				'status' => ($item['quantity'] > 0) ? '1' : '0',  // Set status to 0 if quantity is 0
			];
			
			// Check if the item already exists
			$existing_item = $this->HomeModel->get_room_item($booking_id, $room_id, $item['id']);
			
			if ($existing_item) {
				// Update existing item
				$this->HomeModel->update_room_item($existing_item['id'], $data);
			} else {
				// Insert new item
				$this->HomeModel->insert_room_items($data);
			}
    ...
    if ($booking_id) {
        ...
        if ($data['booking_details']) {
            ...
            $data['guest_names'] = implode(', ', $guest_names); // Join names with commas
			  ...
			  $settlement = $this->HomeModel->getSettlementByBookingId($booking_id);
//var_dump( $settlement);die;
			  if ($settlement) {
				  // If a settlement exists, retrieve the existing invoice number and date
				  $data['invoice_no'] = $settlement['invoice_no'];
				  $data['invoice_date'] = $settlement['invoice_date'];
			  } else {
				  // Generate a new Invoice Number and Date
...
public function settlement_save() {
    ...
    // Extract the settlement_id from the received data
    $settlementId = $settlementData['settlement_id'];

    if ($settlementId) {
        // Settlement exists, update it
        $this->db->where('settlement_id', $settlementId);
        $this->db->update('settlement', [
            'customer_id' => $settlementData['customer_id'],
            'invoice_no' => $settlementData['invoice_no'],
        ...
        // No settlement ID, create a new settlement
        $settlementInsertData = [
            'booking_id' => $settlementData['booking_id'],
            'customer_id' => $settlementData['customer_id'],
            'invoice_no' => $settlementData['invoice_no'],
...
//     try {
//         $booking_id = $this->input->post('booking_id');
//         $item_id = $this->input->post('item_id'); // Get item_id from the request
//         $status = $this->input->post('status');
...
//         // Update the status for the specific item with the booking ID
//         $this->db->where('booking_id', $booking_id);
//         $this->db->where('item_id', $item_id); // Use item_id for unique updates
...
//         }
//     } catch (Exception $e) {
//         log_message('error', 'Failed to update status. Error: ' . $e->getMessage());
...
public function update_item_status1() {
    ...
    try {
        ...
        $status = $this->input->post('status'); // Assume 0 means removed

        // Check if booking_id and item_id are provided
        if (!$booking_id || !$item_id) {
            throw new Exception('Missing booking ID or item ID.');
        }
...
public function update_room_occupy_submit()
{
	date_default_timezone_set('Asia/Kolkata');
	$adding_date=date('Y-m-d H:i:s');
	$selected_rooms = json_decode($this->input->post('selected_rooms'), true);
...
public function get_settlements_by_booking() {
    // Get POST data properly for CodeIgniter
    $bookingId = $this->input->post('booking_id');
    if (!$bookingId) {
        echo json_encode(['success' => false, 'message' => 'Booking ID is required']);
        return;
    }
    // Fetch all settlements for the given booking_id
    $this->db->select('*');
    $this->db->from('settlement');
    $this->db->where('booking_id', $bookingId);
    $settlements = $this->db->get()->result_array();

    if (empty($settlements)) {
        echo json_encode(['success' => false, 'message' => 'No settlements found']);
        return;
    }
