...
public function insert_hotel_room_facility($insertedRoomID, $facilityId) {
    ...
    //     ));
    //     if ($query->num_rows() > 0) {
    //         // Subfacility already exists, update the status
    //         $this->db->where(array(
    //             'hotel_roomid' => $hotelRoomId,
    //             'subfacilityid' => $subfacilityId,
    //             'facilityid' => $facilityId,
    //             'date' => $date
    //         ));
    //         $this->db->update('hotel_room_facility', array('status' => $isChecked));
    //     } else {
    //         // Subfacility does not exist, insert a new record
    //         $data = array(
    //             'hotel_roomid' => $hotelRoomId,
    //             'subfacilityid' => $subfacilityId,
    //             'facilityid' => $facilityId,
    ...
    // }
    // public function getRoomTypesWithRooms1() {
    //     $this->db->select('admin_room.roomtype,hotel_room.room_status AS status,hotel_room.roomno,
    //     hotel_room.hotel_roomid,room_booking.booking_id,room_booking.order_id');
    //     $this->db->from('hotel_room');
    //     $this->db->join('admin_room','admin_room.roomid = hotel_room.roomtypeid');
    ...
    // public function getRoomBookings_booked($hotel_roomid)
    // {
    //     $this->db->select('rb.*, hr.status as room_status, hr.roomtypeid, ar.roomtype, 
    //     hr.roomno, hr.hotel_roomid');
    //     $this->db->from('room_booking rb');
    //     $this->db->join('hotel_room hr', 'rb.hotel_roomid = hr.hotel_roomid');
    //     $this->db->join('admin_room ar', 'ar.roomid = hr.roomtypeid');
    //     $this->db->where('hr.hotel_roomid', $hotel_roomid);
    //     $query = $this->db->get();
    ...
    //     $this->db->select('room_occupy_items.*, room_occupy.room_name, room_occupy.customer_name, room_occupy.customer_email');
    //     $this->db->from('room_occupy_items');
    //     $this->db->join('room_occupy', 'room_occupy.id = room_occupy_items.occupy_id');
    //     $this->db->where('room_occupy_items.occupy_id', $occupy_id);
    //     $query = $this->db->get();
    //     return $query->result();
    // }
